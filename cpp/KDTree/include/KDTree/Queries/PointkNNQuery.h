...
namespace kdtree
{
    ...
    public:
        PointkNNQuery(const Db &db, int k) : db(db), k(k) {}

        PointkNNQuery &search(const Scalar *_query)
        {
            query = _query;
            res.results.clear();
    ...
    private:
        void perform_search(typename Db::Navigator nav, int depth)
        {
            ...
            if (res.results.size() < k || dist2 < res.results.back().distance)
            {

                if (res.results.size() >= 30)
                {
                    // for large number of values it is better to keep the order by inserting at the correct place
                    res.results.emplace(lower_bound(res.results.begin(), res.results.end(), dist2, [](auto &a, auto b)
                                                    { return a.distance < b; }),
                                        dist2,
                                        nav.pointer);
                }
                ...
                {
                    // for small number of values its faster to just resort the whole array
