...
public:
    ...
    MyDbFixture() : pointNNQuery(tree), point2NNQuery(tree, 2), point8NNQuery(tree, 8), pointRadiusQuery(tree, 0.01) {}
...
(benchmark::State &st)
{
    ...
    {
        i = (i++) % numQueries;
        auto &res = pointNNQuery.search(queries.data() + tree.get_dim() * i).getResult();
        benchmark::DoNotOptimize(res);
    }
}
BENCHMARK_REGISTER_F(MyDbFixture, PointNNQuery)->Arg(1e3)->Arg(1e4)->Arg(1e5)->Arg(1e6);
...
(benchmark::State &st)
{
    ...
    {
        i = (i++) % numQueries;
        auto &res = point2NNQuery.search(queries.data() + tree.get_dim() * i).getResult();
        benchmark::DoNotOptimize(res);
    }
}
BENCHMARK_REGISTER_F(MyDbFixture, Point2NNQuery)->Arg(1e3)->Arg(1e4)->Arg(1e5)->Arg(1e6);
...
(benchmark::State &st)
{
    ...
    {
        i = (i++) % numQueries;
        auto &res = point8NNQuery.search(queries.data() + tree.get_dim() * i).getResult();
        benchmark::DoNotOptimize(res);
    }
}
BENCHMARK_REGISTER_F(MyDbFixture, Point8NNQuery)->Arg(1e3)->Arg(1e4)->Arg(1e5)->Arg(1e6);
...
(benchmark::State &st)
{
    ...
    {
        i = (i++) % numQueries;
        auto &res = pointRadiusQuery.search(queries.data() + tree.get_dim() * i).getResult();
        benchmark::DoNotOptimize(res);
    }
}
BENCHMARK_REGISTER_F(MyDbFixture, PointRadiusQuery)->Arg(1e3)->Arg(1e4)->Arg(1e5)->Arg(1e6);
