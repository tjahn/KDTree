...
BENCHMARK_DEFINE_F(MyDbFixture, AlignedRectQuery)
(benchmark::State &st)
{
    int i = 0;
    for (auto _ : st)
    {
        int i0 = (i++) % numQueries;
        i = (i++) % numQueries;
        auto &res = alignedRectQuery.search(queries.data() + tree.get_dim() * i0, queries.data() + tree.get_dim() * i).getResult();
        benchmark::DoNotOptimize(res);
    }
}
BENCHMARK_REGISTER_F(MyDbFixture, AlignedRectQuery)->Arg(1e3)->Arg(1e4);
